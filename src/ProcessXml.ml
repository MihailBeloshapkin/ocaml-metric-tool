...
let process_loc output_el loc =
  ...
  let attrs =
    [ ("", "Lines"), lines; ("", "Logic Lines"), lloc; ("", "Comments"), comments ]
  in
...
let process_holsted output_el holsted =
  ...
  let attrs =
    [ ("", "name"), holsted.func_name
    ; ("", "Volume"), volume
    ; ("", "Theoretical Volume"), theoretical_volume
    ]
  in
  output_el (tag_with_attrs "Function" attrs) ""
;;
...
let out_module out data =
  ...
  let output_module_data () =
    let output_metric name o =
      ...
      out `El_end
    in
    output_metric "CognitiveComplexity" (fun _ ->
      Option.iter ~f:(List.iter ~f:(process_cg output_el)) cogn_data);
    output_metric "CyclomaticComplexity" (fun _ ->
      Option.iter ~f:(List.iter ~f:(process_cc output_el)) cycl_data);
    output_metric "LOC" (fun _ -> Option.iter ~f:(process_loc output_el) loc_data);
    output_metric "Holsted" (fun _ ->
      Option.iter ~f:(List.iter ~f:(process_holsted output_el)) holsted_data)
  in
...
let write_data_to_xml common_data oc =
  ...
  common_data |> List.iter ~f:(out_module out);
  out `El_end
;;
